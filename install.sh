...
genfstab -U /mnt >> /mnt/etc/fstab ;printf "${LRD}Enter Root Password: ${NC}\n" ;passwd ;while [[ $? -ne 0 ]]; do ; passwd
printf "${LRD}Enter User Name: ${NC}" ; read -r USN ;useradd -m -g users -G wheel -s /bin/bash "$(USN)" ;passwd $USN 

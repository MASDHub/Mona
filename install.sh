...
$ARC ln -sf /mnt/usr/share/zoneinfo/$(curl -s https://ipapi.co/timezone) /mnt/etc/localtime ; $ARC hwclock --systohc 
$ARC locale-gen --purge en_US.UTF-8 ; $ARC echo -e LANG="en_US.UTF-8" >> /mnt/etc/locale.conf ; $ARC echo "KEYMAP=us" >> /mnt/etc/vconsole.conf
$ARC echo "${USN}pc" >> /mnt/etc/hostname ; $ARC echo -e "127.0.0.1 localhost \n::1 localhost \n127.0.1.1 ${USN}pc.localdomain ${USN}pc \n" >> /mnt/etc/hosts 

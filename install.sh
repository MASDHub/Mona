...
read -r -p "Enter User Name: " USN ; usermod -m -d /mnt/home/${USN} ${USN} ; echo "${USN} ALL=(ALL) ALL " >> /mnt/etc/sudoers.d/${USN}
genfstab -U /mnt >> /mnt/etc/fstab ; passwd $USN -R /mnt ; echo "Enter Root Password: " ; passwd -R /mnt
echo -e LANG="en_US.UTF-8" >> /etc/locale.conf ; echo "KEYMAP=us" >> /etc/vconsole.conf ; read -r -p "PC Name? " PN 

...
genfstab -U /mnt >> /mnt/etc/fstab ; echo "Enter Root Password: " ; passwd -R /mnt ; read -r -p "Enter User Name: " USN
useradd -m -G wheel -R /mnt $USN ; passwd -R /mnt $USN ; locale-gen --purge en_US.UTF-8 ; echo "${USN} ALL=(ALL) ALL " >> /mnt/etc/sudoers.d/${USN}
read -r -p "PC Name? " PN ; ARC="arch-chroot /mnt" ; echo -e LANG="en_US.UTF-8" >> /etc/locale.conf ; echo "KEYMAP=us" >> /etc/vconsole.conf

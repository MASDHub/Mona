...
if [[ -n "$(cat /etc/locale.gen | grep -o "en_${G}" ]]; then sed -i "s/#${G}/${G}/" /${D}.gen\
&& echo "LANG="$(cat /etc/locale.gen | grep -o "en_${G}"" >> /${D}.conf ; else sed -i "s/#${C}/${C}/" /${D}.gen\
&& echo "LANG=en_US.UTF-8" >> /${D}.conf ; fi ; locale-gen 

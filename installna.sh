...
if [[ "$( pacman -Qd | grep -Ec tpl )" == [1-9] ]] ; then systemctl enable tlp && systemctl enable acpid ; fi
...
echo 'then xrandr --output "${M1}" --mode 1920x1080 --pos 0x0 --rotate normal --output "${M2}" --primary --auto --pos 1920x0 --rotate normal ; fi' ; } >> ${X}
cat /home/${U}/plank/docks.ini | dconf load /net/launchpad/plank/docks/ ; rm /home/${U}/plank/docks.ini

...
printf "${B}DISPLAY MANAGER ENABLED${C}" ; systemctl enable sddm ; X='/usr/share/sddm/scripts/Xsetup' ; { echo 'M="$(xrandr | grep -Ec ''HDMI-1|HDMI1|eDP1|eDP-1|VGA1|VGA-1'')"'
echo 'M1="$(xrandr|grep -Eo ''eDP1|eDP-1|VGA1|VGA-1'')"' ; echo 'M="$(xrandr | grep -Eo ''HDMI-1|HDMI1'')"' echo 'if [[ "${M}" == "2" ]]' 

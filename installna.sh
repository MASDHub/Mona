...
echo 'if [[ "${M}" == "2" ]]' 
echo 'then xrandr --output "${M1}" --mode 1920x1080 --output "${M2}" --primary --auto'  # --rotate normal --rotate normal' --pos 0x0 --pos 1920x0 
echo 'fi' ; } >> ${X} ; sed -i -e "1 s/${E}/" -e "2 s/${F}/" ${G} ; sed -i -e "2 s/${E}/" -e "3 s/${F}/" ${H} 
echo ''"${G}"' "'"${K}"'"' >> ${G} ; echo "${G} ${K}" >> ${H} 
sed -i "s/${I}/" ${H} ; sed -i "s/${I}/" ${G} ; update-desktop-database ; sed -i "s/${D} no/${D} yes/" ${A}

...
lsblk -o name,size,type,mountpoint -e 7,11 ; cp install.sh /mnt/install.sh ; chmod +x /mnt/install.sh #sed -i 's/#Color/Color/' /etc/pacman.conf 
...
pacstrap -i /mnt base base-devel linux-lts linux-lts-headers linux-firmware networkmanager efibootmgr grub-btrfs vim 2>/dev/null #git

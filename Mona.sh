...
read C ; D="/dev/${C}" ; sgdisk "${D}" -Z -o -n 1::+512M -t 1:EF00 -n -i -v -p
D1="$(ls /dev/* | grep -E "^${D}p?1$")" ; D2="$(ls /dev/* | grep -E "^${D}p?2$")"
mkfs.vfat ${D1} ; mkfs.btrfs -fq ${D2} ; E='mount -o noatime,compress=zstd,discard=async,subvol=@' 
F="btrfs su cr @" ; mount ${D2} /mnt ; cd /mnt ; ${F} ; ${F}home ; cd ; umount /mnt ; ${E} ${D2} /mnt

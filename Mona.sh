...
${G} ${E2} /mnt  ;mkdir /mnt/{boot,home}; mount ${E1} /mnt/boot ; ${G}home ${E2} /mnt/home 
${H}-key --populate archlinux>/dev/null ; gpg -k ; ${H}-key --init 
sed -i 's/#Color/Color/' /etc/${H}.conf ; sed -i "s/#${I}/${I}/" /etc/${H}.conf ; lsblk -o ${C} 
...
if [[ -n '${M1}' ]] ; then sed -i "s/${J}()/${J}(${M3})/" ${K} ; else printf "${A}No GPU Detected${B}\n" ; fi
if [[ -n "$(grep -E '[8|9|10|11|12|13|14]' /sys/class/dmi/id/chassis_type)" ]] ; then R='tlp acpid cbatticon'
fi ; curl -L https://raw.githubusercontent.com/djSharcode/Mona/main/install.sh > /mnt/install.sh
...
${P}en-us ${P}fr ${P}de ${P}it ${P}ja ${P}zh-cn ${P}ru ${P}he ${P}pt-br ${O}sans ${O}mono ${R} \

...
mkdir /mnt/{boot,home} ; mount ${E1} /mnt/boot ; ${G}home ${E2} /mnt/home ; lsblk -o ${C}
...
elif [ "${M}" == 'AMD' ] ; then M1="amd-ucode ${L}amdgpu" && M2='amdgpu' ; fi
if [ -n "${M}" ] ; then sed -i "s/${J}()/${J}(${M2} btrfs)/" /etc/mkinitcpio.conf ; fi 

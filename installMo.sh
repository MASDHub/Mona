...
echo "${J}File Manager${K}nemo${L}"
echo "${J}Search${K}rofi -show drun${L}"
echo "${J}Internet${K}firefox-developer-edition${L}"
echo "${J}Terminal${K}alacritty${L}"
echo "${J}Text Editor${K}geany${L}"
echo "${J}Settings${K}nemo${L}"
echo "${J}Calculator${K}galculator${L}"
echo "${J}Refresh${K}openbox --reconfigure${L}"
echo "${J}Reboot${K}reboot${L}"
echo "${J}Lock Screen${K}${L}"
echo "${J}Power Off${K}poweroff now${L}" ; } >> ~/.config/openbox/menu.xml
